...
var getProductionData= async (poultryDBHandler) => {
    return new Promise((resolve, reject) => {
        poultryDBHandler.transaction((tx)=>{
            tx.executeSql("select p.refnos, p.trans_date, ps.pen_name, gt.goods_description, p.quantity from production p inner join pens ps ON ps.pen_id = p.penid inner join goods_table gt ON gt.goods_id = p.productid  order by p.refnos desc", [], 
                function(tx2, result) {
                    console.log(result);
                    var len = result.rows.length, i;
                    ...
                    str += "<thead><tr><th>Date</th><th>Pen</th><th>Product</th><th>Qty</th></tr></thead>";
                    str += "<tbody>";
                    ...
                    for(i=0; i<len; i++){
                        ...
                        str += "<tr><td>"+dated+"</td><td>"+pen_id+"</td><td>"+product+"</td><td>"+qty+"</td><td><button class='btn btn-danger' id='"+ref+"' value='"+del_Qry+"' name='production' onclick='deleteEntry(this.id, this.value, this.name)' >Delete</button></td></tr>";
                        // str += "<tr><td>"+dated+"</td><td>"+ref+"</td><td>"+qty+"</td><td>"+amt+"</td><td><button class='btn btn-success' id='"+ref+"' value='"+del_Qry+"' placeholder='"+poultryDBHandler+"'  onclick=deleteDOCEntry(this.id, this.value, this.placeholder) ><i class='img/b_drop.png'><i/>Delete</button></td></tr>";
...
var saveEggProductionData= async (poultryDBHandler) => {
    ...
    function insertProduction(tx) {
        tx.executeSql('insert into production (trans_date, penid, productid, quantity, amount, user_id, currentweek, flightnumber) values (?, ?, ?, ?, ?, ?, ?, ?)', [dated, penid, product, qty, amt, user_id, wk, "NA" ]);
    }
...
var saveDailyMortalityData= async (poultryDBHandler) => {
    ...
    function insertMortality(tx) {
        tx.executeSql('insert into mortality_hist (pen_id, date, quantity, currentweek, flightnumber) values (?, ?, ?, ?, ?)', [penid, dated, qty, wk, "NA" ]);
    }

...
//     });
// });
var createClientsDependentDBTablesforClients = (resultSet) => {
    // console.log('dele 2')
    return new Promise( async (resolve, reject)=>{
        try {            
            ...
            var prod_sql        ="CREATE TABLE if not exists `production` (refnos integer primary key autoincrement, trans_date text, penid integer, productid integer, quantity integer, amount numeric, user_id text, currentweek integer, flightnumber text, pieces integer, srcdoc text)";
            var acct_sql        ="CREATE TABLE if not exists `general_ledger` (account_id text, account_title text)";    
            var drugusage_sql   ="CREATE TABLE if not exists `drugusage` (refnos integer primary key autoincrement, penid integer, trans_date text, stock_id integer, quantity integer, price numeric, currentweek integer, flightnumber text, srcdoc text)";
            var morta_sql       ="CREATE TABLE if not exists `mortality_hist` (mortality_id integer primary key autoincrement, pen_id integer, date text, quantity integer, currentweek integer, flightnumber text, srcdoc text)";
            ...
            var doc_purchase    ="CREATE TABLE if not exists `broiler_trans_hist` (`purchase_id` integer primary key AUTOINCREMENT, `trans_date` text, `invoice_no` text, `trans_type` text, `amount` numeric, `pen_id` integer, `quantity` integer, `buy_price` numeric, `user_id` text, `supplierid` integer, `goods_id` integer, `age` integer, flightnumber text, srcdoc text)";
            ...
            var feed_usage      ="create table if not exists `feedUsage` (refnos integer primary key autoincrement, pen_id integer, goods_id integer, qty integer, cost_price numeric, dated text, user_id text, flightnumber text, srcdoc text) ";
            var drug_dispensed  ="create table if not exists `drug_dispensed` (refnos integer primary key autoincrement, pen_id integer, goods_id integer, qty integer, cost_price numeric, dated text, user_id text, flightnumber text, srcdoc text) ";
...
let errorLogger=(err)=>{
    ...
    dbHandler = window.sqlitePlugin.openDatabase({name: "poultryDB.db", location: "default"});
    dbHandler.transaction(insertErrorLog, errorCB, successCB);
    var dt = new Date();
    
    function insertErrorLog(tx){
        tx.executeSql('insert into error_logger(error_message, dated, timer) values(?, ?, ?)', [err, dt, dt])
    }
    // error_logger` ( error_message text, dated text, module_id text, timer text ) ";
    function errorCB(err){
